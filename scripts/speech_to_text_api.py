...
from stt_api_client import DEFAULT_STT_MODEL, DEFAULT_TRANSCRIPT_PROMPT, STTApiError, transcribe_audio_file
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Speech-to-text via API Gemini.")
    ...
    parser.add_argument("--model", default=DEFAULT_STT_MODEL, help="Modele Gemini a utiliser.")
    parser.add_argument(
        "--prompt",
        default=None,
        help="Prompt STT personnalise. Si absent, un prompt de transcription francais propre est utilise.",
    )
    parser.add_argument(
        "--link-db",
        action="store_true",
...
def main() -> None:
    ...
    try:
        final_text = transcribe_audio_file(
            audio_path,
            model=args.model,
            prompt=args.prompt or DEFAULT_TRANSCRIPT_PROMPT,
        )
    except STTApiError as exc:
        raise SystemExit(f"\nErreur STT API:\n{exc}") from exc
